...
class AutoFixer {
  ...
  fixFormatting() {
    ...
    const result = this.exec(
      'npx prettier --write "**/*.{js,jsx,ts,tsx,css,md,json}" --ignore-path .gitignore',
      true,
    );
    if (result !== null) {
  ...
  fixTypeScriptErrors() {
    ...
    if (errors && errors.includes("Cannot find name")) {
      this.exec(
        'npx eslint . --fix --rule "import/no-unresolved: error"',
        true,
      );
  ...
  fixAmplifyIssues() {
    ...
    if (!fs.existsSync("amplify_outputs.json")) {
      ...
      try {
        hasAppId = this.exec(
          'grep -r "app-id" .github/workflows/*.yml 2>/dev/null | head -1',
          true,
        );
      } catch (e) {
      ...
      if (hasAppId) {
        ...
        if (match && match[1]) {
          const appId = match[1];
          this.exec(
            `npx ampx generate outputs --app-id ${appId} --branch main --format json`,
            true,
          );
  ...
  async run() {
    ...
    if (this.fixes.length > 0) {
      ...
      if (process.env.CI) {
        ...
        this.exec("git add -A");
        this.exec(
          `git commit -m "🤖 Auto-fix: ${this.fixes.join(", ")}" || true`,
        );
      }
    } else {
    ...
    if (this.errors.length > 0) {
      this.log(
        `${this.errors.length} errors could not be auto-fixed:`,
        "error",
      );
